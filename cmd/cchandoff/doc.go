...
//
// It reads a Claude Code JSONL session or a Gemini chat JSON file and
// produces a structured bootstrap prompt for continuing the work in a
// different tool (claude-code, gemini-cli, codex, ...). The prompt
...
// # Flags
//
//	-from PATH      Source session file (.jsonl or Gemini session-*.json).
//	-to TOOL        Target tool: gemini|gemini-cli|claude|claude-code|codex|codex-cli|codex-app.
